...
class NodeTest extends UnitTestCase {
  ...
  public function testBuildLink(array $target, array $expected, $shouldFailover = FALSE) {
    ...
    if ($shouldFailover) {
      ...
      $expected['#url'] = Url::fromRoute('entity.node.canonical', ['node' => 1], ['language' => NULL]);
  ...
  public function buildLinkProvider() {
    $failoverTarget = ['target' => 'nid:2', 'dest' => '2', 'language' => NULL];
    $failoverExpected = [
      '#theme' => 'freelink_error',
      '#plugin' => 'nid',
      '#message' => 'Invalid node ID @nid',
    ];
    $successTarget = ['target' => 'nid:1', 'dest' => '1', 'language' => NULL];
    $successExpected = [
      '#type' => 'link',
      '#title' => 'Test Node',
      '#attributes' => [
