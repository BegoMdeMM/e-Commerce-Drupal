 ...
 * @group freelinking
 */
class NodeTitleFailoverTest extends UnitTestCase {

  /**
   * Mock container.
   *
   * @property \Drupal\Core\DependencyInjection\Container
   */
  protected $container;
   ...
   *
   * @property \Drupal\Core\StringTranslation\TranslationInterface
   */
  ...
  protected function setUp() {
    ...
    // Mock Entity Query Factory.
    $entityQueryFactoryProphet = $this->prophesize('\Drupal\Core\Entity\Query\QueryFactory');
    $entityQueryFactoryProphet->get('node', 'AND')->willReturn($entityQuery);
    ...
    $container->set('entity.query', $entityQueryFactoryProphet->reveal());
   ...
   */
  public function testFailover($failoverOption, $expected) {
    ...
    if ($failoverOption === 'search') {
      $expected['#url'] = Url::fromUri(
        'base:search/node',
        [
          'language' => NULL,
          'query' => ['keys' => 'Test Node'],
        ]
      );
    }
